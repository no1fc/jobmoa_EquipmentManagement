...
cd mobile && flutter build apk --release   # APK 빌드 (build/app/outputs/flutter-apk/)
...
- **AI 장비 인식**: AiModelService 인터페이스 추상화 (Gemma 4 E2B), 온디바이스 추론, Privacy-First
- **FCM 푸시**: FirebaseMessaging + FcmService (토큰 등록/갱신, 포그라운드/백그라운드 수신)
...
| Users | CRUD + `GET /me` + `PUT /me/fcm-token` | Role: COUNSELOR, MANAGER |
...
- Mobile: `AppConfig.baseUrl` (기본값 `http://58.151.241.130:4590`, `--dart-define`으로 오버라이드 가능)
- Firebase: `google-services.json` (Android), `GoogleService-Info.plist` (iOS) — .gitignore 처리
